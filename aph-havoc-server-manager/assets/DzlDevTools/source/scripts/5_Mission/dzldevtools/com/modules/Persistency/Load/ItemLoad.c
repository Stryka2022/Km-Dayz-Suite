// Ported from COM core/modules/Persistency/Load/ItemLoad.c (unchanged).
class ItemLoad
{
    static EntityAI Load(EntityAI oParent, PlayerBase oPlayer, ItemData oData) {
        ...
        {
            case COM_CARGO_TYPE:
                oEntity = oParent.GetInventory().CreateEntityInCargoEx(oData.Name, 0, oData.X, oData.Y, false);
                break;
            ...
            case COM_GROUND:
                ...
                oEntity = SpawnEntity(oData.Name, dest, ECE_PLACE_ON_SURFACE, RF_DEFAULT);
                break;
