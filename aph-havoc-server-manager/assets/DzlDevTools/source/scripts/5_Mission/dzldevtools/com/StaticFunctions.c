/*
	Ported from DayZCommunityOfflineMode core/StaticFunctions.c.

	PORT CHANGES vs COM original:
	  - COM_GetClientMission() returned CommunityOfflineClient, which no longer
	    exists (that class dissolved into `modded class MissionGameplay`). It now
	    returns MissionGameplay. Verified GetHud()/GetHudPanelWidget() live on
	    MissionGameplay/IngameHud (P:\scripts\5_mission\mission\missiongameplay.c:1235,
	    P:\scripts\5_mission\gui\ingamehud.c:1023) — callers using it (COMKeyBinds
	    HideHud) still work.
	  - COM_GetServerMission() (CommunityOfflineServer) dropped: the server side is
	    left vanilla per port rules, so that cast target no longer exists.
*/
...
// PORT: CommunityOfflineClient dissolved into `modded class MissionGameplay`;
// this now returns MissionGameplay (the offline client mission is a MissionGameplay).
static MissionGameplay COM_GetClientMission()
{
    return MissionGameplay.Cast( GetGame().GetMission() );
}
...
static void COM_SnapToGroundNew( Object object )
{
    ...
    pos[1] = pos[1] + clippingInfo[1][1] / 2.0;
