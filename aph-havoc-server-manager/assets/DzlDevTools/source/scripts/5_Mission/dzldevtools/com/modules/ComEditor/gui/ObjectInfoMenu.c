// Ported from COM core/modules/ComEditor/gui/ObjectInfoMenu.c (unchanged).
class ObjectInfoMenu extends PopupMenu
{
	...
	override bool OnClick( Widget w, int x, int y, int button )
	{
		...
		if ( w.GetName() == "object_editor_info_dumplods" )
		{
			...
			if (!object)
			{
				COM_Message("Dump LODs: no object selected");
				return true;
			}
			object.GetLODS(lods);
			...
			{
				// Vehicles/proxies can report LOD slots with no LOD object behind them (1.29) —
				// dereferencing those crashed the VM.
				if (!lod) continue;
				...
				{
					if (selection) toCopy = toCopy + " " + selection.GetName() + "\n";
				}
			}
