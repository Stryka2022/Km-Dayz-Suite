/*
	Ported from the ALREADY-PATCHED COM copy core/modules/ComEditor/gui/ObjectMenu.c.
	Carries the 1.29 GetMousePos fix: g_Game.GetMousePos(x,y) -> global GetMousePos(x,y)
	(line ~216, OnMouseButtonDown). No further changes.
*/
class ObjectMenu extends PopupMenu
{
	...
	override bool OnMouseButtonDown( Widget w, int x, int y, int button )
	{
		if (w == m_item_widget)
		{
			...
			GetMousePos(m_characterRotationX, m_characterRotationY); // PORT 1.29: was g_Game.GetMousePos(...)
			return true;
		}
		return false;
	}
