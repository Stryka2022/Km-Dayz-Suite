/*
	Ported from COM core/modules/CameraTool/gui/CameraSettings.c.
	PORT CHANGE: CAMERA_ROT / CAMERA_PHI CreateWidgets paths rewritten to the packed
	mod paths.
*/
class CameraSettings extends PopupMenu
{
	...
	override void Init()
    {
		...
		CAMERA_ROT = GetGame().GetWorkspace().CreateWidgets( "DzlDevTools/gui/CameraTool/layouts/CameraROT.layout" );
		CAMERA_PHI = GetGame().GetWorkspace().CreateWidgets( "DzlDevTools/gui/CameraTool/layouts/CameraPHI.layout" );
	}
	...
	void UpdateSliders()
	{
		...
		widgetStore.GetTextWidget("camera_slider_hue_value").SetText( CameraTool.HUESHIFT.ToString() );
		...
		widgetStore.GetSliderWidget("camera_slider_hue").SetCurrent( (CameraTool.HUESHIFT + 5.0) / 0.1);
