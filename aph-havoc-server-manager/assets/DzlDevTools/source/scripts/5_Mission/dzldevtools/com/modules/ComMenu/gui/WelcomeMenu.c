/*
	Ported from COM core/modules/ComMenu/gui/WelcomeMenu.c.
	PORT CHANGE: CreateWidgets layout path rewritten to the packed mod path.
	NOTE: this menu is not auto-registered/opened by any ported module (it wasn't in
	COM either) — kept as a compilable, reachable-on-demand menu class.
*/
class WelcomeMenu extends UIScriptedMenu
{
    ...
    {
        layoutRoot = GetGame().GetWorkspace().CreateWidgets( "DzlDevTools/gui/ComMenu/layouts/WelcomeMenu.layout" );
